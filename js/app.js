...
import { clearProject, loadProject, saveProject } from "./storage.js?v=182";
import { exportNotebookCsv } from "./export.js?v=182";
import {
...
} from "./point-names.js?v=182";
import { initializeAnalytics, trackEvent } from "./analytics.js?v=182";
...
const APP_RELEASE_VERSION = new URL(import.meta.url).searchParams.get("v") || "182";
...
const POINT_NAME_KEYBOARD_ROWS = [
  [..."1234567890"],
  [..."QWERTYUIOP"],
  [..."ASDFGHJKL"],
  [..."ZXCVBNM", "-", ".", "+"]
...
function initializePointNameKeyboard() {
  const fragment = document.createDocumentFragment();
  POINT_NAME_KEYBOARD_ROWS.forEach((keys) => {
    const row = document.createElement("div");
    row.className = "point-name-keyboard-row";
    keys.forEach((key) => {
      const button = document.createElement("button");
      button.type = "button";
  ...
  pointNameKeyboardKeys.replaceChildren(fragment);
}

function updatePointNameKeyboardDisplay() {
...
function openNativePointNameKeyboard() {
  ...
  scheduleFocusedCellKeyboardScroll(target, 80);
}
...
async function finishPointNameKeyboardInput() {
  const target = pointNameKeyboardTarget;
...
async function applyPointSuggestion(pointName) {
  const target = selectedInput?.isConnected && selectedInput.dataset.field === "pointName"
    ? selectedInput
    : null;
  if (!target) return;
  if (voiceSessionActive) cancelActiveVoiceSession();
...
voiceButton.addEventListener("click", async () => {
  if (!pointNameKeyboard.hidden) hidePointNameKeyboard();
  if (!voiceModeActive) {
